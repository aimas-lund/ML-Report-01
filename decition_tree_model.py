...
scores_a = cross_val_score(dec_tree, X_train, y_train, cv=folds)  # calculates x-validation scores
print("X-validation used: %i-folds" % folds)
print("X-validation accuracy: %0.5f (+/- %0.5f)" % (scores_a.mean(), scores_a.std() * 2))
...
folds = 10  # fold for k-folds x-validation
"""
N = X_train.shape[0]  # number of rows in the data set
Y = np.array(X_train - np.ones((N, 1)) * X_train.mean(axis=0), dtype=float) / X_train.std(axis=0)
...
plt.boxplot([scores_a, scores_b])
plt.xticks([1, 2], ["As is", "PCA"])
